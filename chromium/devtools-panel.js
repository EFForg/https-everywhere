...
function display() {
  chrome.runtime.sendMessage({
    ...
    let nrw_div = switchPlannerSmallHtmlSection(response.nrw);
    let rw_text_div = document.createElement("div");
    ...
    let rw_div = switchPlannerSmallHtmlSection(response.rw);
...
function switchPlannerSmallHtmlSection(asset_host_list) {
  ...
  for (let i = asset_host_list.length - 1; i >= 0; i--) {
    let host = asset_host_list[i][3];
    let activeCount = asset_host_list[i][1];
    let passiveCount = asset_host_list[i][2];
