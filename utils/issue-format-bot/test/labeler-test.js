...
vows.describe('issue labeler module').addBatch(setup(
  'and we pass it an issue in the top 100 domains', {
    ...
    'the labels were the top-100 and new-ruleset labels': assertWhichLabels(['top-100', 'new-ruleset']),
    'all other labels were removed': assertOtherLabelsRemoved(['top-1k', 'top-10k', 'top-100k', 'top-1m', 'bug', 'enhancement', 'ruleset-bug', 'ruleset-enhancement'])
  }
)).addBatch(setup(
  'and we pass it an issue in the top 1,000 domains', {
    ...
    'it adds labels to the issue only once': addLabelsCalledOnce,
    'the labels were the top-1k and new-ruleset labels': assertWhichLabels(['top-1k', 'new-ruleset']),
    'all other labels were removed': assertOtherLabelsRemoved(['top-100', 'top-10k', 'top-100k', 'top-1m', 'bug', 'enhancement', 'ruleset-bug', 'ruleset-enhancement'])
  }
)).addBatch(setup(
  'and we pass it a codebase issue', {
    ...
    'all other labels were removed': assertOtherLabelsRemoved(['top-100', 'top-1k', 'top-10k', 'top-100k', 'top-1m', 'enhancement', 'ruleset-bug', 'ruleset-enhancement', 'new-ruleset'])
  }
)).addBatch(setup(
